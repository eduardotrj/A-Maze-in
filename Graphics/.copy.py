...
class MLXCanvas(Canvas):
    ...
    def image_to_memory(self, array: np.ndarray, image: ImgData) -> None:
        ...
        # 1. Asegurar que los datos de la matriz OpenCV estén guardados de
        # forma contigua en memoria y extraer sus bytes crudos
        raw_bytes = np.ascontiguousarray(array, dtype=np.uint8).tobytes()

        # 2. Calcular el tamaño en bytes que requiere almacenar
        # la imagen en MLX
        # (Suele ser alto * bytes por linea de escaneo)
        total_bytes = min(len(raw_bytes), image.height * image.bytesPL)

        # 3. Copiar la memoria de Python directamente al punterode
        # C de MiniLibX ctypes.memmove gestiona de forma automática si
        # image.data es un entero, un puntero o un buffer compatible,
        # copiando los bytes instantáneamente.
        ...
        # buffer = np.frombuffer(image.data,
        #                       dtype=np.uint8).reshape(array.shape)
        # buffer[:, :, :] = array[:, :, :]
        # --------

        # # Obtener el puntero de memoria de tu imagen de MLX
        # img_ptr, bpp, line_length,
        #        endian = self.ve.mlx_get_data_addr(mlx_image_obj.id)
    ...
    def create_image(self, width: int, height: int) -> ImgData:
        ...
        all_info = self.ve.mlx_get_data_addr(image.id)
        image.data, image.bytesPP, image.bytesPL, image.format = all_info
